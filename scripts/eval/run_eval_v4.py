#!/usr/bin/env python3
"""
RAG Eval v4 - 医小管知识库检索质量评测
针对 eval-set-v4.yaml 运行，输出 Markdown 报告
"""
import yaml
import requests
import urllib.parse
...
EVAL_SET = "scripts/eval/eval-set-v4.yaml"
KB_SEARCH_URL = "http://localhost:8000/kb/search"
TOP_K = 5
REPORT_PATH = "scripts/eval/eval-report-v4.md"
...
def run_eval():
    ...
    lines = [
        ...
        f"| 指标 | 数值 | 基线要求 | 状态 |",
        f"|------|------|---------|------|",
        f"| Recall@5（hit题命中率） | {recall_at5:.1%} | ≥ 85% | {'✅' if recall_at5 >= 0.85 else '❌'} |",
    ...
    for r in results:
        if r["id"].startswith("v8-"):
            exp_str = ", ".join(r["expected"][:2]) if r["expected"] else "-"
            got_str = ", ".join(str(g) for g in r["got"][:2]) if r["got"] else "无结果"
            status = "✅" if r["pass"] else "❌"
            lines.append(f"| {r['id']} | {r['q'][:25]}... | {exp_str} | {got_str} | {status} |")
    ...
    if recall_at5 >= 0.85 and reject_acc >= 0.90:
    ...
    print(f"\n{'='*60}")
    print(f"RAG-EVAL v4 完成")
    print(f"  Recall@5: {recall_at5:.1%} ({'PASS' if recall_at5>=0.85 else 'FAIL'})")
